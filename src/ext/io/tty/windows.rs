...
pub struct WindowsTtyPlatform;
impl TtyPlatform for WindowsTtyPlatform {
    type FileDescriptor = std::os::windows::io::RawHandle;
    ...
    fn console_size(fd: Self::FileDescriptor) -> Result<ConsoleSize, std::io::Error> {
        ...
        unsafe {
            ...
            if winapi::um::wincon::GetConsoleScreenBufferInfo(fd, &raw mut bufinfo) == 0 {
